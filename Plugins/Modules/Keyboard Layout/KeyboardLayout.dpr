...
Description: KeyboardLayout Main Project File
...
uses
  ...
  JvSimpleXML,
  // SharpE Units
  SharpApi, SharpCenterApi, SharpESkinManager, SharpEBar, uSharpBarApi, MouseTimer,
  // Project Units
...
begin
  result := 0;
  if ModuleList = nil then exit;
  if CompareText(msg,'MM_INPUTLANGCHANGE') <> 0 then exit;

  for n := 0 to ModuleList.Count - 1 do
      if TModule(ModuleList.Items[n]).ID = ID then
      begin
        temp := TModule(ModuleList.Items[n]);
        TMainForm(temp.FForm).UpdateCurrentLayout;
      end;
end;
