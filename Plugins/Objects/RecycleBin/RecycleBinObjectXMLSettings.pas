{
Source Name: RecycleBinObjectXMlSettings
...
begin
  ...
  FXML.Root.Name := 'RecylcBinObjectSettings';
...
begin
  ...
  Dir := UserDir + 'SharpDesk\Objects\RecycleBin\';
...
begin
  ...
  begin
    SharpApi.SendDebugMessageEx('RecycleBin.object','Settings File does not exist',0,DMT_INFO);
  end;
  ...
  try
    ...
    SharpApi.SendDebugMessageEx('RecycleBin.object',PChar('Failed to load Settings File: '+Settingsfile),0,DMT_ERROR);
  end;
  ...
  begin
    ...
    Size             := Value('Size','48');
    IconFileF        := Value('IconFileF','icon.application');
    IconFileE        := Value('IconFileE','icon.application');
    ...
    ShowData         := BoolValue('ShowData',False);
...
begin
  ...
  SaveSetting(FXMLRoot,'IconFileF',IconFileF,False);
  SaveSetting(FXMLRoot,'IconFileE',IconFileE,False);
  ...
  SaveSetting(FXMLRoot,'ShowData',ShowData,True);
  ...
  begin
    ...
    try
      ...
      SharpApi.SendDebugMessageEx('RecycleBin.object',PChar('Failed to save Settings to: '+SettingsDir+'~temp.xml'),0,DMT_ERROR);
    ...
    if not RenameFile(SettingsDir+'~temp.xml',SettingsFile) then
       SharpApi.SendDebugMessageEx('RecycleBin.object','Failed to Rename Settings File',0,DMT_ERROR);
  end;
end;
