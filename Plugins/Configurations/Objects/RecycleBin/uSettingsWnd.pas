...
type
  TfrmSettings = class(TForm)
    ...
    procedure edtRecycleBinFullChange(Sender: TObject);
...
procedure TfrmSettings.chkBinStatusClick(Sender: TObject);
begin
  SendUpdate;
...
procedure TfrmSettings.edtRecycleBinFullChange(Sender: TObject);
begin
  SendUpdate;
end;
...
var
  s: string;
begin
  s := SharpDialogs.IconDialog(edtRecycleBinFull.Text, SMI_ALL_ICONS, Mouse.CursorPos);
  if length(trim(s)) > 0 then
  begin
    edtRecycleBinFull.Text := s;
    UpdateIcon;
  end;
end;

procedure TfrmSettings.PngSpeedButton2Click(Sender: TObject);
var
  s: string;
begin
  s := SharpDialogs.IconDialog(edtRecycleBinEmpty.Text, SMI_ALL_ICONS, Mouse.CursorPos);
  if length(trim(s)) > 0 then
  begin
    edtRecycleBinEmpty.Text := s;
    UpdateIcon;
  end;
end;
