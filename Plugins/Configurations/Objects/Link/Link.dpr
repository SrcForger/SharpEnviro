...
begin
  ATabItems.AddObject('Link', frmSettings.pagLink);
...
var
  xmlSettings: TLinkXMLSettings;
  ITheme: ISharpETheme;
  s: string;
  n: integer;
begin
  xmlSettings := TLinkXMLSettings.Create(strtoint(PluginHost.PluginId), nil, 'Link');
  try
    ...
    begin
      ...
      edtTarget.Text := xmlSettings.Target;
      {$ENDREGION}
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        MLineCaption := False;
        Caption := edtSingle.Text;
      end;
      ShowCaption := chkCaption.Checked;
      xmlSettings.Icon := edtIcon.Text;
      xmlSettings.Target := edtTarget.Text;
      {$ENDREGION}
...
procedure TSharpCenterPlugin.SetDefaults(xmlSettings: TLinkXMLSettings;
  ITheme: ISharpETheme);
begin
