...
begin
  ...
  with FSharpESwatchCollection do
  begin
    Align := alClient;
...
begin
  ...
  if FColorCode < 0 then
    col := SchemeCodeToColor(FColorCode)
  else
    col := FColorCode;
...
begin
  ...
  if Assigned(FSwatchManager) then
  begin
...
begin
  ...
  if ((Sender = FRedSlider) or (Sender = FBlueSlider) or (Sender = FGreenSlider))
    then
  begin
